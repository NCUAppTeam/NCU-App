...
function HomeScreen({ navigation }) {
  ...
  const onRefresh = () => {
    setRefreshing(true);
    SalesController.getAllItems().then((res) => {
      setProduct(res);
      // setSearchbarText("");
      setTimeout(() => {
        setRefreshing(false);
  ...
  useEffect(() => {
    SalesController.getItembyType(0)
      .then((res) => {
        setSellProduct(res);
      })
      .catch((err) => {
  ...
  useEffect(() => {
    SalesController.getItembyType(1)
      .then((res) => {
  ...
  useEffect(() => {
    SalesController.getItembyType(2)
      .then((res) => {
        setRentProduct(res);
      })
      .catch((err) => {
  ...
  function AcquireproductCards() {
    return Acquireproduct.map(({ productName, price, imageURL }) => (
  ...
  function SellproductCards() {
    return Sellproduct.map(({ productName, price, imageURL }) => (
      <Box alignItems="center" marginTop="5">
        <Box
          maxW="250"
          rounded="lg"
  ...
  function RentproductCards() {
    return Rentproduct.map(({ productName, price, imageURL }) => (
      <Box alignItems="center" marginTop="5">
        <Box
          maxW="250"
          rounded="lg"
      ...
      <SafeAreaView style={styles.container}>
        <HStack alignItems="center" w="95%" justifyContent="center">
          ...
          <Feather
            ...
            style={styles.icon}
            onPress={() => {
              navigation.navigate('拍賣個人管理頁面', { onGoBack: onRefresh });
            }}
          />
        </HStack>
        <HStack
