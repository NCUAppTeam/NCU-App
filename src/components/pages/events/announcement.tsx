    ...
    if (!isQualified) {
        ...
        return (
            <div className="flex flex-col items-center justify-center h-60">
                ...
                <p className="text-gray-500 text-lg text-center">您非正取參加者，暫不開放通知區</p>
            </div>
        );
    }
        ...
        <div className='grid gap-y-2 bg-white p-4 m-4 rounded-lg h-108'>
            ...
            <p className='text-gray-500'>
                {props.isHost ? (
                    ...
                    <span className='bg-gradient-to-r from-[#FF5F6D] via-[#FFC371] to-[#FBB03B] text-transparent bg-clip-text font-extrabold'>
                        你已錄取為正取。揪人創辦者會在此發布重要通知，你可以進行回覆。
                    </span>
                )}
            </p>
            {announcements.length > 0 ? (
                <div className='grid gap-y-2 overflow-y-auto max-h-48 h-fit'>
                    {announcements.map((chat, index) => (
                        <div key={chat.id ?? index} className='p-2 bg-gray-100 rounded-lg h-fit w-full'>
                            <div className='flex flex-row items-center gap-2 mb-2'>
                                ...
                                <div className='flex flex-col'>
                                    <div className='flex items-center gap-2'>
                                        <p className='font-bold text-sm'>{chat.members?.username}</p>
